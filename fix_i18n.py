...
# Chinese content to add
chinese_content = """    // GAD-7 Content
...
# Find the Chinese result levels marker (first occurrence)
zh_marker = "    // Result levels\n    'scl90_level_normal': '正常',"
if zh_marker in content:
    content = content.replace(zh_marker, chinese_content + zh_marker, 1)
    print("OK: Added Chinese content")
else:
    print("FAIL: Could not find Chinese marker")

# English content to add - find second occurrence
en_marker = "    // Result levels\n    'scl90_level_normal': 'Normal',"
count = content.count(en_marker)
print(f"Found English marker {count} times")

if count >= 2:
    parts = content.split(en_marker)
    content = parts[0] + en_marker + english_content + en_marker + parts[2]
    print("OK: Added English content")
else:
    print(f"FAIL: Only found {count} occurrence(s)")
